...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch mosquito_$target-$variant
        fi
    fi
    return $?
}
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop ro.mosquito.device | grep -q "$STAG_BUILD"); then
...
function installboot()
{
    ...
    if (adb shell getprop ro.mosquito.device | grep -q "$STAG_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.mosquito.device | grep -q "$STAG_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
